...
// Define the pin for the PIR sensor's output on PB4
#define PIR_SENSOR_PIN GPIO_PIN_4
#define PIR_SENSOR_PORT GPIOB

// Define the built-in LED pin on PA5
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if (HAL_GPIO_ReadPin(PIR_SENSOR_PORT, PIR_SENSOR_PIN) == GPIO_PIN_SET) {
		  ...
		  HAL_Delay(1000);  // Keep laser on for 1 second
		  HAL_GPIO_WritePin(LASER_PORT, LASER_PIN, GPIO_PIN_RESET);

	  } else {
		  // Turn off the built-in LED if no motion is detected
